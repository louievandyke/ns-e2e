param(
    [string]$nomad_sha,
    [string]$nomad_version = 1.3.2,
    [string]$nomad_binary,
    [switch]$enterprise = $false,
    [switch]$nomad_acls = $false,
    [string]$config_profile = dev-cluster,
    [string]$role,
    [string]$index = 0,
...
function InstallConfigProfile {
    ...
    if (!(Test-Path -Path $PROFILE)) {
        Write-Progress -ItemType File -Path $PROFILE -Force
      }
      
    Write-Progress $PROFILE '$ProgressPreference = "SilentlyContinue"'
